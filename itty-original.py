...
# XXX make a combined url search regex

def make_decorator(verb):
    def deco(url):
        def register(method):
            re_url = re.compile("^%s$" % add_slash(url))
            REQUEST_MAPPINGS[verb].append((re_url, url, method))
            return method
        return register
    deco.__name__ = verb.lower()
    deco.__doc__ = "Register a method as capable of processing %s requests." % verb
    return deco

get, post, put, delete = map(make_decorator, ['GET', 'POST', 'PUT', 'DELETE'])
...
def run_itty(server='wsgiref', host='localhost', port=8080, config=None):
    ...
    try:
        ...
        print 'Shutting down. Have a nice day!'import cgi
import mimetypes
import os
import re
import StringIO
